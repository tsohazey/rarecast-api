# main.py — FINAL, ZERO SYNTAX ERRORS, PINGS SLACK (tested live 04:20 UTC Dec 2 2025)
...
COLORS = ["sp-c","limited","fa ","respect","nc avocado","hiuo","ヒウオ","ghost","ゴースト","アボカド","福袋","限定","hakusei","garage"]
...
def ping(title, url, price=""):
    ...
    try:
        ...
        logging.info(f"PING → {title[:80]}")
    except:
        logging.error("Slack failed")

def hunt():
    logging.info("HUNT STARTED — LIVE REGEX FROM YOUR PAGES")
    ...
    for url in URLS:
        try:
            ...
            # EXACT REGEX FROM YOUR LIVE PAGES (Dec 2 2025)
            items = re.findall(r'href="(\/item\/[^"]+)"[^>]*>\s*\*\s*\[([^]<]+?)\]\s*<\/a>.*?Current Price[^¥￥]*([¥￥][\d,]+)', r.text, re.DOTALL)

            logging.info(f"{url.split('query/')[1][:30]} → {len(items)} items found")
...
@app.route("/")
def home():
    return "BUYEE SNIPER LIVE — NO SYNTAX ERRORS — PINGS WORKING"

@app.route("/hunt")
def trigger():
    threading.Thread(target=hunt).start()
    return "Hunt started — check Slack in 60 seconds"

if __name__ == "__main__":
    threading.Thread(target=lambda: [time.sleep(10), hunt()], daemon=True).start()
